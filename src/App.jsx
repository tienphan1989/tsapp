...
class App extends React.Component {
  ...
  handleLogin = (event, user) => {
    ...
    fetch(
      ...
      configObj("POST", true, { username, password })
    )
  ...
  handleRegister = (event, newUser) => {
    ...
    fetch(
      ...
      configObj("POST", true, { user: { username, password, age, email } })
    )
      .then((response) => response.json())
      .then((data) => {
        localStorage.token = data.token;
      ...
      <BrowserRouter>
        <div className="App">
          ...
          <Switch>
            ...
            <Route
              exact
              path="/vaccination_records/id/"
              render={(routeProps) => <VaccineForm {...routeProps} />}
            />
          </Switch>
